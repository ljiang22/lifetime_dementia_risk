...
def sample_app():
    ...
    user2 = user1[['Age', 'apoe', 'M/F', 'Hand', 'Education', 'Race', 'LIVSIT', 'INDEPEND', 'RESIDENC', 'MARISTAT',
...
def doc_app():
    ages = np.arange(30, 111, 1)
    pif = {
           'Age': ages,
           'Gender': ['male', 'female']
           }

    st.sidebar.markdown("# Input your patient's infos")
    Age = st.sidebar.number_input(label="What is your age? (30 - 110)", min_value=30.0, max_value=110.0, step=1.0, format='%d')

    'The patient age is:', Age
...
def users_app():
    ...
    independ = st.sidebar.selectbox("What is the level of independence? ",
                                 ['Able to live independently', 'Requires some assistance with complex activities', 'Requires some assistance with basic activities', 'Completely dependent', 'Other'])

    if independ == 'Able to live independently':
        independ = 0.0
    elif independ == 'Requires some assistance with complex activities':
        independ = 1.0
    elif independ == 'Requires some assistance with basic activities':
        independ = 2.0
    ...
    user_info_ori = [commun, homehobb, judgment, memory, orient, perscare, cdr_clc, age, gene, gender, hand, edu, race, livsit, independ, residenc, maris, BMI, dem_idx, CVHATT, CVAFIB, CVANGIO, CVBYPASS, CVPACE, CVCHF, CVOTHR, CBSTROKE, CBTIA, CBOTHR, PD, PDOTHR, SEIZURES, TRAUMBRF, TRAUMEXT, TRAUMCHR, NCOTHR, HYPERTEN, HYPERCHO, DIABETES, B12DEF, THYROID, INCONTU, INCONTF, DEP2YRS, DEPOTHR, ALCOHOL, TOBAC30, TOBAC100, SMOKYRS, PACKSPER, ABUSOTHR, PSYCDIS, GDS]
    user_info_ori = np.reshape(user_info_ori, (1, len(user_info_ori)))
    keys = ['commun', 'homehobb', 'judgment', 'memory', 'orient', 'perscare', 'sumbox', 'Age', 'apoe', 'M/F', 'Hand', 'Education', 'Race', 'LIVSIT', 'INDEPEND', 'RESIDENC', 'MARISTAT',
     'BMI', 'dem_idx', 'CVHATT', 'CVAFIB', 'CVANGIO', 'CVBYPASS', 'CVPACE', 'CVCHF', 'CVOTHR', 'CBSTROKE', 'CBTIA',
     'CBOTHR', 'PD', 'PDOTHR', 'SEIZURES', 'TRAUMBRF', 'TRAUMEXT', 'TRAUMCHR', 'NCOTHR', 'HYPERTEN', 'HYPERCHO',
     'DIABETES', 'B12DEF', 'THYROID', 'INCONTU', 'INCONTF', 'DEP2YRS', 'DEPOTHR', 'ALCOHOL', 'TOBAC30', 'TOBAC100',
     'SMOKYRS', 'PACKSPER', 'ABUSOTHR', 'PSYCDIS', 'GDS']
